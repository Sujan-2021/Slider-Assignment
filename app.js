...
const showImages = (images) => {
  if(images.length === 0){
    alert('Item Not Available in Our DB');
    return;
  }
...
const getImages = (query) => {
  ...
  fetch(`https://pixabay.com/api/?key=${KEY}=${query}&image_type=photo&pretty=true`)
    ...
    .catch(err => {
      alert("An error occurred while fetching data. Please try again sometime.");
      toggleLoader();
      return;
    })
}
...
const createSlider = () => {
  ...
  if(duration < 0){
    alert("Slider duration cannot be Negative. Please Enter a Positive Value to create a slider");
    return;
  }
  clearInterval(timer);
...
//to get search work on Enter press
...
// Toggle addSpinner
const toggleLoader =()=>{
  document.getElementById('addSpinner').classList.toggle('d-none');
}
